...
class DataFirstDao:
    ...
    async def get_cars_above_avg_mileage(cls, limit: int = 10):
        ...
        async with async_session_maker() as session:
            ...
            response = [SCarScheme(**row._asdict()) for row in result.fetchall()]
            return response
    ...
    async def join_request(cls, limit: int = 10):
        ...
        async with async_session_maker() as session:
            result = await session.execute(request)
            response = [
                SRequestTripWithScore(**row._asdict()) for row in result.fetchall()
            ]
            return response
    ...
    async def cte_request(cls, limit: int = 10):
        ...
        async with async_session_maker() as session:
            result = await session.execute(request)
            response = [
                SDriverTripStats(**row._asdict()) for row in result.fetchall()
            ]
            return response
    ...
    async def metadata(cls):
        ...
        async with async_session_maker() as session:
            result = await session.execute(request)
            response = [SMetadata(**row._asdict()) for row in result.fetchall()]
            return response
    ...
    async def get_trips_by_price(cls, low: int, high: int):
        ...
        async with async_session_maker() as session:
            result = await session.execute(request)
            response = [
                STripsDriverPassengerInfo(**row._asdict())
